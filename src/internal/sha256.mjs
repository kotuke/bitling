/**
 * Minimal SHA-256 and HMAC-SHA-256 over Uint8Array, with no imports at all.
 * The figure builders need a hash that also works in a browser, so the shared
 * core cannot reach for `node:crypto`.
 */
...
export function sha256(message) {
  ...
  for (let offset = 0; offset < padded.length; offset += BLOCK_SIZE) {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      const s1 = rotateRight(ahead, 17) ^ rotateRight(ahead, 19) ^ (ahead >>> 10);
...
/** HMAC-SHA-256 over byte arrays. Returns 32 bytes. */
export function hmacSha256(key, message) {
  ...
  for (let index = 0; index < BLOCK_SIZE; index += 1) {
    const byte = index < normalizedKey.length ? normalizedKey[index] : 0;
